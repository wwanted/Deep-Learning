...
class Darknet(nn.Module):
    def __init__(self, cfgfile):
        ...
        self.num_calsses = int(self.blocks[-1]["classes"])
        self.anchors = self.blocks[-1]["anchors"].split(",")
        self.anchors = [int(a) for a in self.anchors]
    ...
    def forward(self, x, CUDA=False, Train=False):
        ...
        for i in range(len(modules)):
            ...
            if module_type == "convolutional" or module_type == "upsample" or module_type == "maxpool":
                ...
                if Train:
                    x, box_b, pred = predict_transform(x, inp_dim, anchors, num_classes, CUDA, Train)
                else:
                    x = predict_transform(x, inp_dim, anchors, num_classes, CUDA, Train)
