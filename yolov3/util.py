...
def predict_transform(prediction, inp_dim, anchors, num_classes, CUDA=True, train=False):
    ...
    prediction_train = prediction.clone()
    ...
    box_a = prediction.new(prediction.shape)[:, :, :4]
    ...
    if train:
        return prediction_train, box_a, prediction
    return prediction
...
def write_results(prediction, confidence, num_classes, nms=True, nms_conf=0.4):
    ...
    for ind in range(batch_size):
        ...
        if len(img_classes) == 0:
            continue
        '''  
        logo_classes = torch.FloatTensor([])
        try:
            img_classes = [x for x in img_classes if x in logo_classes]
        except:
            continue
            
        img_classes = torch.FloatTensor(img_classes)
        '''
